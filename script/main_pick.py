...
def main(task_file, out_file):
    ...
    rospy.loginfo("starting pick task from %s" % task_file)
    ...
    main_strategy = pick_process.select_strategy(pos_info)
    ...
    if not raw_input("This will send commands to the robot. " +
        ...
        return
    # start run
    task_items = [(obj["item"], obj["bin"]) for obj in task["work_order"]]
    try:
        main_strategy.execute(task_items)
...
if __name__ == "__main__":
    ...
    parser.add_argument("file",
                        help="input file for the pick task")
