...
# Global settings
out_image_dir = './result/segmentation_out_images10_8'
out_model_dir = './result/segmentation_out_models10_8'
...
def get_image_and_label():
    ...
    print 'Shuffling...'
    xs, ys = sklearn.utils.shuffle(xs, ys, random_state=0)

    # use 90% as training samples and 10% as test samples
    Nt = int(0.9 * xs.shape[0])
    print '# of training samples: {}'.format(Nt)
    print '# of test samples: {}'.format(xs.shape[0] - Nt)
    return xs[:Nt], ys[:Nt], xs[Nt:], ys[Nt:]

def crop_and_mirror(img, cont, test=True):
    ...
    img, cont = img[:, oh:(oh+224), ow:(ow+224)], cont[oh:(oh+224), ow:(ow+224)]
    if mirror_flag == 1:
        img, cont = img[:,:,::-1], cont[:,::-1]

    if test:
        return img, cont
    else:
        return add_noise(img, cont)

def add_noise(image, cont):
    ...
    def rotate(img, cont):
        ...
        if flag < 6:
            ...
            return np.rot90(img, k=1), np.rot90(cont, k=1)
        else:
            return np.rot90(img, k=3), np.rot90(cont, k=3)

    img = ((image + 1) * 128).astype(np.uint8).transpose([1, 2, 0])

    img = white_noise(gaussian_blur(hsv_filter(img)))
    img, cont = rotate(img, cont)
...
#        layers['dc3_1'] = L.Deconvolution2D(64, 64, 3, 1, 1, wscale=0.02*math.sqrt(3*3*64))
        ...
        layers['dc4_1'] = L.Deconvolution2D(64, 40, 3, 1, 1, wscale=0.02*math.sqrt(3*3*64))
...
#        h = F.leaky_relu(self.bn3_1(self.dc3_1(h), test=test))
        h = F.leaky_relu(self.bn3_2(self.dc3_2(h), test=test))
        h = F.leaky_relu(self.bn4_0(self.dc4_0(h), test=test))
        h = (self.dc4_1(h))
        return h


class TDecoder(chainer.Chain):
...
#        layers['dc3_1'] = L.Deconvolution2D(64, 64, 3, 1, 1, wscale=0.02*math.sqrt(3*3*64))
        layers['dc3_2'] = L.Deconvolution2D(64, 64, 4, 2, 1, wscale=0.02*math.sqrt(4*4*64))
        layers['dc4_0'] = L.Deconvolution2D(64, 64, 3, 1, 1, wscale=0.02*math.sqrt(3*3*64))
        layers['dc4_1'] = L.Deconvolution2D(64, 2, 3, 1, 1, wscale=0.02*math.sqrt(3*3*64))
        layers['bn0_0'] = L.BatchNormalization(256)
        layers['bn0_1'] = L.BatchNormalization(128)
        layers['bn1_0'] = L.BatchNormalization(128)
        layers['bn1_1'] = L.BatchNormalization(128)
        ...
        layers['bn3_2'] = L.BatchNormalization(64)
        layers['bn4_0'] = L.BatchNormalization(64)
        super(TDecoder, self).__init__(**layers)
...
def make_batch(imgs_train, imgs_test, labels_train, labels_test, test):
    x = np.zeros((batchsize, 4, 224, 224)).astype(np.float32)
    t = np.zeros((batchsize, 224, 224)).astype(np.int32)
    for i in range(batchsize):
        if not test:
            r = np.random.randint(imgs_train.shape[0])
            img, cont = imgs_train[r], labels_train[r]
            v = False
        else:
            r = np.random.randint(imgs_test.shape[0])
            img, cont = imgs_test[r], labels_test[r]
            v = True

        img_c, cont_c = crop_and_mirror(img, cont, test=test)
        x[i,:] = img_c
        t[i,:] = cont_c
    return Variable(xp.asarray(x), volatile=v), Variable(xp.asarray(t), volatile=v)


def train(enc, dec, initial_epoch=0):
    import matplotlib
    matplotlib.use('Agg')
    import matplotlib.pyplot as plt

    o_enc = optimizers.Adam(alpha=0.001*(0.98**initial_epoch))
    o_dec = optimizers.Adam(alpha=0.001*(0.98**initial_epoch))
    o_enc.setup(enc)
    o_dec.setup(dec)
    decay_enc = 0.0001
    decay_dec = 0.0001
    ...
    imgs_train, labels_train, imgs_test, labels_test = get_image_and_label()
    ...
    for epoch in range(initial_epoch, 10000):
        ...
        for it in range(0, 10000, batchsize):
            print epoch, it
            x, t = make_batch(
                imgs_train, imgs_test, labels_train, labels_test, test=False)
            z = enc(x, test=False)
            y = dec(z, test=False)
            yd = Variable(y.data)
            ...
            y.backward()
            #o_enc.clip_grads(10.0)
            #o_dec.clip_grads(10.0)
            o_enc.update()
            o_dec.update()
            ...
            if (it+batchsize)%log_itv == 0:
                ...
                for i_test in range(1):
                    x, t = make_batch(
                        imgs_train, imgs_test, labels_train, labels_test, test=True)
                    y = dec(enc(x, test=True), test=True)
...
if __name__ == '__main__':
    ...
    enc = Encoder()
    serializers.load_npz('/data/user/apc2016/segmentation_out_models9_4/model_enc_128.npz', enc)

    dec = Decoder()
    serializers.load_npz('/data/user/apc2016/segmentation_out_models9_4/model_dec_128.npz', dec)
    ...
    mkdir(out_image_dir)
    mkdir(out_model_dir)
    shutil.copy('segmentation_train.py', out_model_dir)

    train(enc, dec, initial_epoch=0)
